...
const commandBuilder = new SlashCommandBuilder()
  ...
  .setDescription(
    "Créez une salle de draft et obtenez les URLs pour les équipes rouge et bleue."
  )
...
const execute = async (interaction) => {
  ...
  try {
    ...
    await interaction.reply({
      content: `Blue Team URL (${data.blue.name}): ${blueTeamUrl}\nRed Team URL (${data.red.name}): ${redTeamUrl}`,
      ephemeral: true,
    });
  } catch (error) {
    ...
    await interaction.reply({
      content:
        "Une erreur est survenue lors de la création de la salle de draft.",
      ephemeral: true,
    });
  }
};
