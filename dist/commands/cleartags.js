...
export const buttons = {
    cleartags_confirm: async (interaction) => {
        ...
        await interaction.editReply({
            content: "User roles have been cleared, except for 'Moderator' and 'Host' roles.",
            components: [],
        });
    },
...
const commandBuilder = new SlashCommandBuilder()
    .setName("cleartags")
    .setDescription("Removes all user roles except for those with a 'Moderator' or 'Host' role.");
const execute = async (interaction) => {
    ...
    if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({
            content: "Are you sure you want to execute the `cleartags` command? This will remove all roles from users, except for those with a 'Moderator' or 'Host' role. This action cannot be undone.",
            components: [row],
        });
    }
};
