...
import { ActionRowBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ButtonBuilder, } from "discord.js";
...
const buttonsRow = new ActionRowBuilder().addComponents(confirmButton, cancelButton);
        ...
        try {
            ...
            for (const elm of selections) {
                ...
                const { error } = await supabase
                    ...
                    .eq("id", elm.id);
                await createApprovedChannel(interaction.guild, elm.name, "1109480250108289124", "1080911803854356670");
                if (error) {
...
const execute = async (interaction) => {
    ...
    const options = inscriptions
        .map((inscription) => new StringSelectMenuOptionBuilder()
