...
const execute = async (interaction) => {
    ...
    for (const adminId of ADMIN_USER_IDS) {
        try {
            ...
            await adminUser.send(`Help request from <@${interaction.user.id}> in <#1052660022800293908>`);
        }
        catch (error) {
            console.error(`Could not send help request to admin with ID ${adminId}.`, error);
        }
    }
    // Reply to the user to confirm that the help request has been sent
    await interaction.reply('Your help request has been sent to the admins.');
};
export default {
