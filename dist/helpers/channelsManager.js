...
export const createApprovedChannel = async (guild, teamName, textCategoryId, voiceCategoryId) => {
...
export const deleteApprovedChannel = async (guild, teamName, textCategoryId, voiceCategoryId) => {
    console.log("guild:", guild);
    console.log("teamName:", teamName);
    try {
        await deleteRole(guild, teamName);
        // Find and delete the text channel
        const textChannel = guild.channels.cache.find(channel => channel.name === teamName && channel.parentId === textCategoryId);
        if (textChannel) {
        ...
        const voiceChannel = guild.channels.cache.find(channel => channel.name === teamName && channel.parentId === voiceCategoryId);
        if (voiceChannel) {
